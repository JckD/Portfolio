...
const ProjectCard = () => {
  ...
  `);
    console.log(data.projects.nodes[0].data.githublink)

  return (
    <>
    {data.projects.nodes.map((item, i)  =>(
      <div className='projectCard'>
      <Image src={item.data.imageURL} className='projectCardImage'/>
      <hr/>
      <h5>{item.data.Title}</h5>
      <p style={{fontSize: '14px'}}>{item.data.description}</p>
      <a href={item.data.githublink} target="_blank" >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#29a19c" class="bi bi-github" viewBox="0 0 16 16">
